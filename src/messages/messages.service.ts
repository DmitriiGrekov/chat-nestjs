import { BadRequestException, HttpException, HttpStatus, Injectable, CACHE_MANAGER, Inject } from '@nestjs/common';
...
export class MessagesService {
  ...
  async send(createMessageDto: CreateMessageDto, userId: number): Promise<Message> {
    try {
      const room = await this.roomService.findOne({
        where: { AND: [{ id: createMessageDto.room_id }, { users: { some: { id: userId } } }] },
      })
      if (!room) throw new HttpException('Вы не являетесь участником данной комнаты', HttpStatus.BAD_REQUEST)
      return await this.prismaService.message.create({ data: { ...createMessageDto, user_id: userId } })
    } catch (error) {
  ...
  async remove(id: number, user_id: number): Promise<Message> {
    try {
      const message = await this.prismaService.message.findFirst({ where: { user_id, id } })
